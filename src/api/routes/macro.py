...
def get_macro_catalog():
    """Series grouped by country/group with labels (before /{series:path} so it
    isn't matched as a series)."""
    return {
        group: [{"code": code, "label": label} for code, label in items]
        for group, items in FRED_SERIES_CATALOG.items()
    }
...
def get_macro_series_route(
    ...
    cache_key = f"macro:{series}:{start}:{end}:{limit}:{offset}:{order}"
    ...
    rows = get_macro_series(db, series, start=start, end=end, limit=limit, offset=offset, order=order)
